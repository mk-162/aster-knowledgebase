---
title: "Verified (pro) teams"
parent: "Teams & clubs"
nav_order: 12
audience: [athlete, organiser]
tags: [teams, pro, verified]
summary: "Verified status for professional and elite teams — what it is, how to apply, and how it differs from the Club plan."
last_updated: 2026-07-02
---

# Verified (pro) teams

Verified status marks a team as a genuine professional or elite outfit. It's
a **badge of identity, reviewed by Aster** — not a paid plan, and not the
same thing as the [Club plan](billing-and-your-club-plan.md).

## Verified vs the Club plan

| | Verified (pro) | Club plan |
|---|---|---|
| What it is | Identity verification for pro/elite teams | A paid subscription for club tools |
| How you get it | Apply with evidence; Aster reviews | Subscribe from the club page |
| What it gives | Verified badge on the team and its riders; sponsor strip on the team page | Weekly rides, unlimited members, sponsor slot, unlimited follows for members |

A team can be either, both, or neither — a pro team doesn't need the Club
plan for its badge, and your local club doesn't need to be "pro" to
subscribe.

## Apply for verified status

1. Open your team's page — only the **team owner** can apply.
2. Choose the pro/verified application and describe why the team qualifies
   (at least a few sentences), with links to evidence — UCI registration,
   race results, press, the team site.
3. Aster reviews the application and you'll hear back either way. If the
   team is already verified, there's nothing to apply for.

## What members get

Riders on a verified team carry the pro flag on their own profiles while
they're on the roster, and the team can show its
[sponsor strip](club-visibility-and-sharing.md) on its page.
