...
A **ping** is a single GPS point — your location at one moment. The app
records pings as you move and sends them to Aster, which connects them into
your trail and adds up the distance between them. If you lose signal, pings
...
When an organiser creates an event they pick a format, and the format decides
how timing, results and finishing work:

- **Race** — every athlete's clock runs from the gun; first across the line
  wins. Ranked results.
- **Time trial** — timed from crossing the start line to crossing the finish
  line, ranked fastest-first.
...
Every ride is bounded by two moments: when it **started** and when it
...
- **Public** — listed in Discover and watchable by anyone.
- **Unlisted** — not listed anywhere, but watchable by anyone who has the link.
- **Private** — you must be signed in to view it, and you need the event code
  to join as an athlete.
- **Members-only** — visible only to active members of the club or team
  hosting it. Never appears in Discover.
...
events and organisers can all have one. Handles make profiles easy to link to
and let you **@mention** someone in a post or comment. See
[Your profile and handle](../account-and-privacy/your-profile-and-handle.md).
